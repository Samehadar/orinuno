...
12. **Source events stream (ADR 0017 Stage B)**: `SourceEventController.getReadyEvents()` under `GET /api/v1/source-events/ready?updatedSince=…&limit=20` — open consumers (the external aggregator's `external bridge` in the external aggregator (out of repo), future OSS aggregators) drive incremental polling here. The controller calls `ExportDataService.findReadyForExportAsEvents(...)` which reuses the same query as the legacy `/api/v1/export/ready` (only contents with at least one decoded mp4 link) and pipes each `(KodikContent, List<KodikEpisodeVariant>)` tuple through `SourceEventMapper.toEvent(...)`. Output is `MovieDiscovered` for movie-shaped types (`movie` / `foreign-movie` / `soviet-cartoon` / `cartoon`), `SeriesDiscovered` for everything else, or `TitleObserved` if every variant got filtered out as expired. **Wire-form invariant**: `event.identifier().sourceId() = String.valueOf(content.getId())` — orinuno PK, NOT `kodikId`, to preserve meter's existing dedup key during the cutover. **Posters (ARCH-0017-FOLLOWUP-POSTER)**: `SourceContentInfo` carries producer-side URLs `posterUrl` / `bigPosterUrl` / `screenshotUrls` / `trailerUrls`. Kodik fills `posterUrl` from `material_data.poster_url_original` (with fallbacks to `poster_url` / `anime_poster_url` / `drama_poster_url`); `bigPosterUrl` is `poster_url_original` only; `screenshotUrls` is the L1 `screenshots` JSON array; `trailerUrls` stays empty (Kodik doesn't expose them). Consumers download to their own storage (the external aggregator's `external bridge` does this via a `PosterAttachments` record passed to `MeterSourceBridge.bridge(event, attachments)`). The legacy `/api/v1/export/ready` (proprietary `ContentExportDto`) stays operational throughout this PR and is removed only once downstream consumer is fully migrated (ARCH-0017 Stage D, follow-up).
