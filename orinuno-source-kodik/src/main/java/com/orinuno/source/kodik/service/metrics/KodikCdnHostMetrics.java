/*
 * KodikCdnHostMetrics — ADR 0021 §D1b-prep.
 *
 * Ported from orinuno-app/.../service/metrics/. Metric name rebased
 * from orinuno.kodik.cdn.host to orinuno.source.kodik.cdn.host so the
 * two coexisting copies (orinuno-app + source-kodik) publish distinct
 * time series during the Block D migration window.
 */
package com.orinuno.source.kodik.service.metrics;
...
public class KodikCdnHostMetrics {
    ...
    static final String METRIC_NAME = "orinuno.source.kodik.cdn.host";
    ...
    private Counter buildCounter(String host) {
        if (counters.size() >= MAX_DISTINCT_HOSTS) {
            log.warn(
                    "Kodik CDN host metric cardinality cap reached ({}). Folding host '{}' into"
                            + " '{}' bucket.",
                    MAX_DISTINCT_HOSTS,
                    host,
                    OVERFLOW_LABEL);
