 ...
 * producer-side {@link SourceCatalogEvent} suitable for exposure on the wire to the external aggregator's{@code
 * external bridge} and other open consumers. Stage B of ARCH-0017.
 *
 * <p>Mapping decisions:
 *
 * <ul>
 ...
 *       column.
 *   <li><b>Movie vs series</b>: same whitelist as {@code downstream consumer}'s {@code KodikMeterMapper}
 *       ({@code movie} / {@code foreign-movie} / {@code soviet-cartoon} / {@code cartoon}).
 *   <li><b>Variant filter</b>: only variants with a non-blank {@code kodikLink} (the long-lived
 *       iframe URL — not the TTL-bound {@code mp4Link}) are surfaced. ADR 0018 Phase 2.6 — this
 *       per-source service does not own the L2 decoded-URL store (that lives in orinuno-app until
 *       Phase 5 moves it into the OSS meter shared catalog DB), so the consumer pulls a fresh
 *       decode against the iframe URL when it actually needs the bytes. See TECH_DEBT ARCH-0018 for
 *       the JIT-decode trajectory. Episodes without playable variants are dropped, empty seasons
 *       are dropped; everything-dropped falls back to {@link SourceCatalogEvent.TitleObserved}.
 ...
 */
@Slf4j
public final class KodikSourceEventMapper {

    private static final String SOURCE_TYPE = "kodik";
    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    private KodikSourceEventMapper() {}
    ...
    private static Map<String, Object> parseMaterialData(String json) {
        ...
        try {
            ...
            log.warn(
                    "KodikSourceEventMapper: failed to parse material_data JSON, posters will be"
                            + " omitted: {}",
                    e.getMessage());
            return null;
        }
    ...
    private static List<String> parseScreenshots(String json) {
        ...
        try {
            ...
            log.warn(
                    "KodikSourceEventMapper: failed to parse screenshots JSON, screenshotUrls will"
                            + " be omitted: {}",
                    e.getMessage());
            return Collections.emptyList();
        }
