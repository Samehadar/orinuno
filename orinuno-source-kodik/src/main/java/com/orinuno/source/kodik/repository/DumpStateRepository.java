...
/**
 * MyBatis mapper for {@code orinuno_dump_state} (DUMP-1, restored ADR 0021 §D5). Idempotent upsert
 * pattern: callers always invoke {@link #upsertCheckResult} after a poll; the row is created on
 * first sight (one per dump_name) and overwritten on every subsequent poll.
 */
@Mapper
