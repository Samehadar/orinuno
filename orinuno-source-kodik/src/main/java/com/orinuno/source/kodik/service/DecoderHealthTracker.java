/*
 * DecoderHealthTracker — ADR 0021 §D1b-prep.
 *
 * Micrometer counters + last-failure snapshot for the Kodik decoder.
 * Ported verbatim from orinuno-app's DecoderHealthTracker. Metric names
 * (`kodik.decoder.success` / `kodik.decoder.failure`) stay identical —
 * source-kodik becomes the canonical owner of these metrics after D3
 * retires the orinuno-app copy. Until then both publish (different JVMs
 * in full-split topology, so no metric collision in practice).
 */
