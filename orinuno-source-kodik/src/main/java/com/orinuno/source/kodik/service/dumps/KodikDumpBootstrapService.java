...
import com.orinuno.source.kodik.mapper.EntityFactory;
import com.orinuno.source.kodik.model.KodikContent;
import com.orinuno.source.kodik.model.KodikEpisodeVariant;
import com.orinuno.source.kodik.service.ContentWriteService;
 ...
 *
 * <p>Originally lived in orinuno-app's {@code service/dumps/} package; ADR 0021 §D5 ported the
 * slice into orinuno-source-kodik unchanged behind the new {@code orinuno.source-kodik.dumps.*}
 * property prefix.
 *
 ...
 * ContentWriteService#findOrCreateContent(KodikContent)} + {@link
 * ContentWriteService#saveVariants(java.util.List)}. This means the bootstrap path automatically
...
public class KodikDumpBootstrapService {
    ...
    private final ContentWriteService contentService;
    private final HttpClient httpClient;

    public KodikDumpBootstrapService(
            KodikDumpsProperties properties, ContentWriteService contentService) {
     ...
     *
     * @throws IllegalStateException if {@code orinuno.source-kodik.dumps.download-body=false} (the
     *     safety gate) or the dump name is unknown.
    ...
    private void ensureDownloadEnabled() {
        if (!properties.isDownloadBody()) {
            throw new IllegalStateException(
                    "Dump body downloads are disabled. Set"
                        + " orinuno.source-kodik.dumps.download-body=true to allow bootstrap (this"
                        + " gates against multi-GB accidental fetches).");
        }
    }

    private DumpEntry resolveEntry(String dumpName) {
        return switch (Optional.ofNullable(dumpName).orElse("").toLowerCase()) {
            case "calendar" -> properties.getCalendar();
            case "serials" -> properties.getSerials();
            case "films" -> properties.getFilms();
            default ->
                    throw new IllegalStateException(
                            "Unknown dump name: '"
                                    + dumpName
    ...
    private HttpResponse<InputStream> openStream(String url)
        ...
        long timeoutSeconds = Math.max(60, properties.getRequestTimeoutSeconds() * 4);
        HttpRequest request =
