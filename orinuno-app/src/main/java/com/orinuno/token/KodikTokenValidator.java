...
public class KodikTokenValidator {
     ...
     *
     * <p>DEAD-tier entries are also re-probed, but only once per {@code
     * orinuno.kodik.dead-revalidation-interval-minutes} (default 24h). This guards against the
     * historical failure mode where a single transient network glitch during the first {@code
     * validate-on-startup} run would permanently exile a token: previously DEAD was skipped
     * entirely, and once a token sat there nothing ever brought it back. {@link
     * KodikTokenRegistry#markValid(String, KodikFunction)} handles the symmetrical promote path
     * (DEAD → UNSTABLE) when any probe succeeds.
     */
    public void validateAll() {
        java.util.Map<KodikTokenTier, List<KodikTokenEntry>> snapshot = registry.snapshot();
        List<KodikTokenEntry> targets = new java.util.ArrayList<>();
        snapshot.forEach(
                (tier, entries) -> {
        ...
        List<KodikTokenEntry> deadCandidates =
                eligibleDeadCandidates(snapshot.get(KodikTokenTier.DEAD));
        targets.addAll(deadCandidates);
        log.info(
                "Revalidating {} Kodik token(s) (live={}, dead-cooldown={})",
                targets.size(),
                targets.size() - deadCandidates.size(),
                deadCandidates.size());
        boolean first = true;
        for (KodikTokenEntry entry : targets) {
            if (!first) {
                sleepQuiet(PROBE_SPACING);
            }
            first = false;
    ...
    /**
     * Returns {@code true} if the next {@link #validateAll()} call would actually probe at least
     * one token. Used by {@link KodikTokenLifecycle} so an all-DEAD registry that has at least one
     * cooldown-eligible candidate still triggers the revival path on startup / scheduled tick.
     */
    public boolean hasAnythingToValidate() {
        ...
        for (KodikTokenTier tier : KodikTokenTier.values()) {
            ...
            if (!snapshot.get(tier).isEmpty()) {
                return true;
            }
        }
        return !eligibleDeadCandidates(snapshot.get(KodikTokenTier.DEAD)).isEmpty();
    }

    private List<KodikTokenEntry> eligibleDeadCandidates(List<KodikTokenEntry> dead) {
        ...
        java.time.Instant cutoff =
                java.time.Instant.now().minus(Duration.ofMinutes(cooldownMinutes));
        List<KodikTokenEntry> eligible = new java.util.ArrayList<>();
        for (KodikTokenEntry entry : dead) {
            java.time.Instant lastChecked = entry.getLastChecked();
            if (lastChecked == null || lastChecked.isBefore(cutoff)) {
                eligible.add(entry);
            }
        }
        return eligible;
