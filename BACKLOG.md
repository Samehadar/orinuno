...
**Референс:** parser-upstream-source в downstream-repo использует RabbitMQ + State Machine. Для standalone-сервиса предпочтительнее REST-based job polling.
...
#### IDEA-KW-1: Token auto-discovery

**Приоритет:** Низкий
**Сложность:** Средняя

kodikwrapper умеет извлекать API-токен из публично доступных страниц, использующих Kodik player. Это может быть полезно как fallback, если токен не задан в конфиге.

**Реализация:**
1. GET запрос к известному сайту с Kodik player (shikimori, etc.)
2. Regex-парсинг HTML для извлечения `token=...` из iframe URL
3. Кэширование найденного токена
4. Использование как fallback при `orinuno.kodik.token` = пусто

**Риск:** Хрупкий подход, зависит от структуры внешних сайтов.
...
| 19 | IDEA-KW-1: Token auto-discovery | Низкий | Средняя | — |
