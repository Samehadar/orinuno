...
| `filepath` / `posterFilepath` / `bigPosterFilepath` / `previewImageFilepath` / `trailerFilepaths` (semantically MinIO object keys in Kin) | yes | rename to `mediaUrl` / `posterUrl` / `bigPosterUrl` / `previewImageUrl` / `trailerUrls`. Meaning becomes "fully-qualified URL". Meter's `external bridge` (consumer-side adapter, lives in external aggregator) re-interprets as a MinIO key on the external aggregator'sside. |
...
| `external bridge` in `external aggregator` (consumes `SourceCatalogEvent`, emits `ContentExportRequest`) | out of scope, follow-up in `external aggregator` |
