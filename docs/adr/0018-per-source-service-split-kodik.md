...
| 5.* | `meter` OSS service; catalog write-path moves into `meter`; `orinuno` read-only repository + Caffeine cache; multi-instance `orinuno`; DB user separation; full split docker-compose | 🚧 in progress (5.1, 5.2a, 5.3, 5.4, 5.5, 5.6, 5.7, 5.7a, 5.9, 5.10, 5.11 ✅; 5.8 + 5.12 + 5.13 ⏳) |
