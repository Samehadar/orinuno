...
4. **Add Prometheus counter** `orinuno_decoder_quality_picked_total{quality}` — incremented in `ParserService.decodeForVariant()` after `selectBestQuality` returns. This gives us a histogram of "max-quality buckets observed in the wild" so we can spot the day Kodik introduces 1080p (the day a `quality="1080"` bucket appears) without anyone asking us to add it. This counter is added in DECODE-6 (which already touches the decoder metrics).
5. The "rename `mp4_link` → `media_link`" idea is parked indefinitely. Renaming the column would break every downstream contract (export DTO, MyBatis XML, OpenAPI spec, Astro docs, kodik-parser consumer expectations, and the `mp4_link_decoded_at` companion column) for cosmetic reasons. The Javadoc on the column already says "Direct media URL — usually .m3u8 (HLS), historically .mp4". That is enough.
