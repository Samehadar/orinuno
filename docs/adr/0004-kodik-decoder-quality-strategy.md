...
5. The "rename `mp4_link` → `media_link`" idea is parked indefinitely. Renaming the column would break every downstream contract (export DTO, MyBatis XML, OpenAPI spec, Astro docs, downstream consumer consumer expectations, and the `mp4_link_decoded_at` companion column) for cosmetic reasons. The Javadoc on the column already says "Direct media URL — usually .m3u8 (HLS), historically .mp4". That is enough.
