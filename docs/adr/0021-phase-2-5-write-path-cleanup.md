...
| D5 — relocate `KodikDumpBootstrapService` + `orinuno_dump_state` changeset (A3 decision) | ✅ this commit — restored the full DUMP-1/DUMP-2 slice (4 service classes + model + repository + MyBatis mapper + Liquibase changeset + 3 test classes + fixture, ~1.2 KLOC) into orinuno-source-kodik with packages rebased onto `com.orinuno.source.kodik.*`. New `KodikDumpsProperties` (prefix `orinuno.source-kodik.dumps.*`) replaces the retired `OrinunoProperties.DumpsProperties`. Dormant by default (`enabled=false`, `download-body=false`); the two-flag safety gate against multi-GB CDN fetches is preserved verbatim. 15/15 ported dump tests pass; whole source-kodik suite green. |
| E1 — ArchUnit guard against Kodik imports in orinuno-app | ✅ this commit — `BoundedContextArchitectureTest.orinuno_app_does_not_reach_into_kodik_sdk_internals` allow-lists `com.kodik.drift..` + `com.kodik.client.http..` + `com.kodik.client.embed..` and bans everything else under `com.kodik..` from orinuno-app `src/main`. Negative probe (a temporary `KodikApiClient` import in `DriftDetectorConfig`) was added locally and verified to trip the guard before being reverted. |
...
| E3 — README + AGENTS.md sync to final shape | ✅ this commit — README badge bumped Spring Boot 3.4 → 3.5; module table now reflects orinuno-app's thin-gateway role + adds `orinuno-source-jutsu` + the meter L2/L3 cutover; monolith profile section rewritten around the per-source `@ConditionalOnProperty` reverse-proxy filters + `@ConditionalOnBean(catalogReadJdbcTemplate)` read-side gates. AGENTS.md rewritten end-to-end: replaces the stale "Controller → Service → Repository → MyBatis" diagram with the per-service ownership table, swaps the legacy "Key Flows" enumeration for a where-is-X module map + properties ownership table, picks up ADR 0018/0019/0020/0021 in the lineage, and ports the gateway-level allow-list (com.kodik.drift / client.http / client.embed) from the E1 ArchUnit guard into the cross-cutting rules section. |
