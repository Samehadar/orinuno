...
class ContentControllerTest {
    ...
    void shouldReturnContentList() {
        ContentDto dto = ContentDto.builder()
                .id(1L).title("Test Movie").type("movie").year(2024).build();

        PageResponse<ContentDto> page = PageResponse.<ContentDto>builder()
                .content(List.of(dto))
        ...
        webTestClient.get().uri("/api/v1/content")
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.content[0].title").isEqualTo("Test Movie")
    ...
    void shouldReturnContentById() {
        ContentDto dto = ContentDto.builder()
                .id(1L).title("Test").type("movie").build();

        when(contentService.findById(1L)).thenReturn(Optional.of(dto));

        webTestClient.get().uri("/api/v1/content/1")
                .exchange()
                .expectStatus().isOk()
                .expectBody()
    ...
    void shouldReturn404ForNonExistent() {
        ...
        webTestClient.get().uri("/api/v1/content/999")
                .exchange()
    ...
    void shouldFindByKinopoiskId() {
        ContentDto dto = ContentDto.builder()
                .id(1L).title("Test").kinopoiskId("123456").build();

        when(contentService.findByKinopoiskId("123456")).thenReturn(Optional.of(dto));

        webTestClient.get().uri("/api/v1/content/by-kinopoisk/123456")
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.kinopoiskId").isEqualTo("123456");
    }
}
