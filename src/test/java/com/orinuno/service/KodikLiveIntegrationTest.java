...
/**
 * Live integration test that calls the real Kodik API.
 * Requires KODIK_TOKEN environment variable to be set.
 * Skipped automatically in CI if token is not provided.
 */
@Testcontainers
...
class KodikLiveIntegrationTest {
    ...
    void healthShouldReturnUp() {
        webTestClient.get().uri("/api/v1/health")
                ...
                .jsonPath("$.service").isEqualTo("orinuno");
    }
    ...
    void searchNarutoByShikimoriId() {
        ...
        webTestClient.post().uri("/api/v1/parse/search")
                ...
                .jsonPath("$[0].kinopoiskId").value(kpId -> {
                    if (kpId != null) {
    ...
    void contentListShouldNotBeEmpty() {
        webTestClient.get().uri("/api/v1/content?page=0&size=10")
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                ...
                .jsonPath("$.content.length()").value(len -> {
    ...
    void getContentByIdShouldReturnData() {
        ...
        webTestClient.get().uri("/api/v1/content/{id}", savedContentId)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.id").isEqualTo(savedContentId.intValue())
    ...
    void variantsShouldExist() {
        ...
        webTestClient.get().uri("/api/v1/content/{id}/variants", savedContentId)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$").isArray()
                ...
                .jsonPath("$[0].translationTitle").isNotEmpty();
    }
    ...
    void exportShouldReturnStructuredData() {
        ...
        webTestClient.get().uri("/api/v1/export/{contentId}", savedContentId)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.id").isEqualTo(savedContentId.intValue())
    ...
    void findByKinopoiskIdShouldWork() {
        ...
        webTestClient.get().uri("/api/v1/content/by-kinopoisk/{kpId}", savedKinopoiskId)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.kinopoiskId").isEqualTo(savedKinopoiskId)
                .jsonPath("$.title").isNotEmpty();
    }
    ...
    void contentListShouldHaveMultipleEntries() {
        webTestClient.get().uri("/api/v1/content?page=0&size=50")
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.totalElements").value(total -> {
    ...
    void decoderHealthShouldBeAccessible() {
        webTestClient.get().uri("/api/v1/health/decoder")
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.successRate").isNotEmpty()
                .jsonPath("$.totalDecoded").isNotEmpty()
                .jsonPath("$.totalFailed").isNotEmpty();
    }
}
