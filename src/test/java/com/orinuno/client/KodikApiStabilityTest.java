...
class KodikApiStabilityTest {
    ...
    private static final Set<String> PAGINATED_KNOWN_FIELDS = Set.of(
            "time", "total", "prev_page", "next_page", "results"
    );
    private static final Set<String> RESULT_REQUIRED_FIELDS = Set.of("id", "link", "translation");
    private static final Set<String> RESULT_KNOWN_FIELDS = Set.of(
            "id", "type", "link", "title", "title_orig", "other_title",
            "translation", "year", "last_season", "last_episode", "episodes_count",
            "kinopoisk_id", "imdb_id", "shikimori_id", "worldart_link",
            "quality", "camrip", "lgbt", "created_at", "updated_at",
            "screenshots", "seasons", "material_data", "blocked_countries",
            "blocked_seasons", "mdl_id", "worldart_animation_id", "worldart_cinema_id"
    );
    private static final Set<String> TRANSLATION_KNOWN_FIELDS = Set.of(
            "id", "title", "type", "count", "count_for_not_anime"
    );
    private static final Set<String> MATERIAL_DATA_KNOWN_FIELDS = Set.of(
            "title", "anime_title", "title_en", "other_titles", "other_titles_en",
            "other_titles_jp", "anime_license_name", "anime_licensed_by",
            "anime_kind", "all_status", "anime_status", "drama_status",
            "year", "description", "poster_url", "screenshots",
            "duration", "countries", "all_genres", "genres", "anime_genres",
            "drama_genres", "anime_studios", "rating", "kinopoisk_rating",
            "kinopoisk_votes", "imdb_rating", "imdb_votes",
            "shikimori_rating", "shikimori_votes", "mydramalist_rating",
            "mydramalist_votes", "premiere_world", "premiere_ru", "premiere_country",
            "aired_at", "released_at", "next_episode_at", "episodes_total",
            "episodes_aired", "actors", "directors", "producers", "writers",
            "composers", "editors", "designers", "operators",
            "rating_mpaa", "minimal_age", "anime_description",
            "poster_url_original", "mydramalist_tags",
            "blocked_countries", "blocked_seasons",
            "anime_poster_url", "drama_poster_url", "tagline"
    );
    private static final Set<String> SIMPLE_RESPONSE_KNOWN_FIELDS = Set.of("time", "total", "results");
    ...
    void searchByTitle(String title) throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=" + encode(title) + "&limit=3");
    ...
    void searchByShikimoriId(String shikimoriId) throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&shikimori_id=" + shikimoriId + "&limit=5");
    ...
    void searchByKinopoiskId(String kpId) throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&kinopoisk_id=" + kpId + "&limit=3");
    ...
    void searchByImdbId(String imdbId) throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&imdb_id=" + imdbId + "&limit=3");
    ...
    void searchWithSeasons() throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&shikimori_id=20&with_seasons=true&with_episodes=true&limit=1");
    ...
    void searchByType(String type) throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=a&types=" + type + "&limit=2");
    ...
    void searchByTranslationType() throws Exception {
        ...
        if (!results.isEmpty()) {
            @SuppressWarnings("unchecked")
            Map<String, Object> translation = (Map<String, Object>) results.get(0).get("translation");
    ...
    void searchExcludeCamrip() throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=Avengers&camrip=false&limit=3");
        ...
        for (Map<String, Object> r : results) {
            assertSoftly(soft -> soft.assertThat(r.get("camrip"))
    ...
    void searchNonexistentContent() throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=xyznonexistent999zzz&limit=1");
    ...
    void searchMaterialData(String shikimoriId, String label) throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&shikimori_id=" + shikimoriId + "&with_material_data=true&limit=1");
        ...
        Map<String, Object> first = results.get(0);
        Assumptions.assumeTrue(first.containsKey("material_data"), "material_data not returned for " + label);
        ...
        assertSoftly(soft -> {
            soft.assertThat(md.containsKey("shikimori_rating") || md.containsKey("kinopoisk_rating"))
    ...
    void searchMaterialDataDramaFields() throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=" + encode("Squid Game") + "&with_material_data=true&limit=5");
    ...
    void searchMaterialDataForMovie() throws Exception {
        ...
        assertSoftly(soft -> {
            soft.assertThat(md.containsKey("kinopoisk_rating")).as("Movie should have kinopoisk_rating").isTrue();
            soft.assertThat(md.containsKey("imdb_rating")).as("Movie should have imdb_rating").isTrue();
            if (md.containsKey("duration")) {
                soft.assertThat(md.get("duration")).as("duration must be a number").isInstanceOf(Number.class);
            }
        });
    ...
    void listWithSortOptions(String sort, String order) throws Exception {
        Map<String, Object> raw = postApi("/list",
                "token=" + token + "&limit=3&sort=" + sort + "&order=" + order);
    ...
    @Order(21)
    @ValueSource(strings = {"anime", "anime-serial", "foreign-movie", "russian-serial", "foreign-serial"})
    void listByType(String type) throws Exception {
        Map<String, Object> raw = postApi("/list",
                "token=" + token + "&types=" + type + "&limit=2&sort=updated_at&order=desc");
        ...
        List<Map<String, Object>> results = (List<Map<String, Object>>) raw.get("results");
        assertThat(results)
                .as("/list types=%s should return results", type)
                .isNotEmpty();
    }
    ...
    void listPagination() throws Exception {
        Map<String, Object> raw = postApi("/list",
                "token=" + token + "&limit=1&sort=updated_at&order=desc");
    ...
    void listWithMaterialData() throws Exception {
        Map<String, Object> raw = postApi("/list",
                "token=" + token + "&limit=10&sort=updated_at&order=desc&with_material_data=true");

        assertPaginatedResponse(raw, "/list with_material_data");

        @SuppressWarnings("unchecked")
        List<Map<String, Object>> results = (List<Map<String, Object>>) raw.get("results");
        boolean anyHasMaterialData = results.stream()
                .anyMatch(r -> r.containsKey("material_data") && r.get("material_data") != null);
        assertThat(anyHasMaterialData)
                .as("with_material_data=true: at least one of %d results should have material_data", results.size())
                .isTrue();
    }
    ...
    void listBySubtitles() throws Exception {
        Map<String, Object> raw = postApi("/list",
                "token=" + token + "&translation_type=subtitles&limit=3&sort=updated_at&order=desc");
    ...
    void translationsStructure() throws Exception {
        ...
        assertSoftly(soft -> {
            ...
            soft.assertThat(first.get("id")).as("id must be a number").isInstanceOf(Number.class);
            soft.assertThat(first.get("title")).as("title must be a string").isInstanceOf(String.class);
        });
    ...
    void translationsKnownVoicesExist() throws Exception {
        ...
        assertSoftly(soft -> {
            soft.assertThat(titles.stream().anyMatch(t -> t.toLowerCase().contains("anilibria")))
    ...
    void genresKnownValuesExist() throws Exception {
        ...
        assertSoftly(soft -> {
            soft.assertThat(genres.size()).as("should have multiple genres").isGreaterThan(3);
        });
    }
    ...
    void genresWithType(String genreType) throws Exception {
        Map<String, Object> raw = postApi("/genres",
                "token=" + token + "&genres_type=" + genreType);
    ...
    void countriesStructure() throws Exception {
        ...
        List<?> results = (List<?>) raw.get("results");
        assertThat(results)
                .as("countries should not be empty")
                .isNotEmpty();
    }
    ...
    void yearsContainRecentYears() throws Exception {
        ...
        for (Object item : results) {
            if (item instanceof Number n) {
                ...
                try { years.add(Integer.parseInt(s)); } catch (NumberFormatException ignored) {}
    ...
    void qualitiesStructure() throws Exception {
        ...
        List<?> results = (List<?>) raw.get("results");
        assertThat(results)
                .as("qualities should not be empty")
                .isNotEmpty();
    }
    ...
    void searchLimitOne() throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=Naruto&limit=1");

        List<?> results = (List<?>) raw.get("results");
        assertThat(results.size()).as("limit=1 should return at most 1 result").isLessThanOrEqualTo(1);
    }
    ...
    void searchStrict() throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=Naruto&strict=true&limit=5");
    ...
    void searchNotBlockedIn() throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=Naruto&not_blocked_in=RU&limit=3");
    ...
    void searchBlockedSeasonsType() throws Exception {
        Map<String, Object> raw = postApi("/search",
                "token=" + token + "&title=Naruto&limit=50");
        ...
        for (Map<String, Object> r : results) {
            if (r.containsKey("blocked_seasons")) {
                ...
                assertThat(bs == null || bs instanceof Map)
                        .as("blocked_seasons should be null or a Map, got %s in '%s'",
                                bs != null ? bs.getClass().getSimpleName() : "null", r.get("title"))
                        .isTrue();
            }
        }
        System.out.println("[INFO] blocked_seasons present in " + withField + "/" + results.size() + " results");
    }
        ...
        for (Map<String, Object> r : results) {
            if (r.containsKey("blocked_countries")) {
                ...
                assertThat(bc == null || bc instanceof List)
                        .as("blocked_countries should be null or a List, got %s in '%s'",
    ...
    void searchInvalidToken() throws Exception {
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(BASE_URL + "/search?token=INVALID_TOKEN_123&title=test&limit=1"))
        ...
        assertSoftly(soft -> {
            for (String field : RESULT_REQUIRED_FIELDS) {
                soft.assertThat(first.containsKey(field))
                        .as("[%s] Result required field '%s' must be present", context, field)
                        .isTrue();
            }
            soft.assertThat(first.get("id")).as("[%s] id must be a string", context).isInstanceOf(String.class);
            soft.assertThat(first.get("link")).as("[%s] link must be a string", context).isInstanceOf(String.class);
            soft.assertThat(first.get("translation")).as("[%s] translation must be a map", context).isInstanceOf(Map.class);
        });
    ...
    private static void detectUnknownKeys(String context, Set<String> actual, Set<String> known) {
        ...
        if (!unknown.isEmpty()) {
            System.err.println("[SCHEMA DRIFT] " + context + ": new unknown fields detected: " + unknown);
        }
    }
}
