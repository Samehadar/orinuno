...
public class KodikApiRateLimiter {
    ...
    public <T> Mono<T> wrapWithRateLimit(Mono<T> apiCall) {
        return Mono.defer(() -> {
            ...
            log.warn("Kodik API rate limit reached ({}/min), waiting for refill...", maxPermits);
            return Mono.delay(Duration.ofSeconds(2))
                    .then(Mono.defer(() -> {
                        ...
                        return Mono.error(new RuntimeException("Kodik API rate limit exceeded, try again later"));
                    }));
        });
    }
