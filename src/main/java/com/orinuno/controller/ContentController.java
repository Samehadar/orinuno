...
public class ContentController {
    ...
    private static final Set<String> ALLOWED_SORT_FIELDS = Set.of(
            "id", "title", "year", "created_at", "updated_at", "kinopoisk_id", "type");
    ...
    public ResponseEntity<PageResponse<ContentDto>> findAll(
        ...
        String safeOrder = ALLOWED_ORDERS.contains(order.toUpperCase()) ? order.toUpperCase() : "ASC";
    ...
    public ResponseEntity<ContentDto> findByKinopoiskId(@PathVariable String kinopoiskId) {
        return contentService.findByKinopoiskId(kinopoiskId)
                .map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }
}
