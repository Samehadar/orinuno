...
public class HlsController {
    ...
    public Mono<ResponseEntity<Map<String, String>>> getHlsUrl(@PathVariable Long variantId) {
        ...
        return hlsManifestService.getHlsUrl(variantId)
                .map(result -> ResponseEntity.ok(Map.of("url", result.url())))
                .onErrorResume(e -> {
                    log.error("HLS URL failed for variant_id={}: {}", variantId, e.getMessage());
    ...
    @Operation(summary = "Get absolutized m3u8 manifest for a variant (fresh decode + download + absolutize)")
    public Mono<ResponseEntity<String>> getAbsolutizedManifest(@PathVariable Long variantId) {
        ...
        return hlsManifestService.getAbsolutizedManifest(variantId)
                .map(result -> ResponseEntity.ok()
                        .contentType(MediaType.parseMediaType("application/vnd.apple.mpegurl"))
                        .body(result.manifest()))
                .onErrorResume(e -> {
                    log.error("HLS manifest failed for variant_id={}: {}", variantId, e.getMessage());
                    return Mono.just(ResponseEntity.badRequest()
