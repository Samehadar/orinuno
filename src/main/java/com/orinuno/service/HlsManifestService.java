...
public class HlsManifestService {
    ...
    public Mono<HlsResult> getHlsUrl(Long variantId) {
        KodikEpisodeVariant variant = episodeVariantRepository.findById(variantId)
                .orElse(null);
        if (variant == null) {
        ...
        return decoderService.decode(variant.getKodikLink())
                .map(videoLinks -> {
                    ...
                    if (best == null) {
                        throw new RuntimeException("No video links decoded for variant: " + variantId);
                    }
        ...
        return getHlsUrl(variantId)
                .flatMap(result -> {
                    ...
                    return proxyWebClientService.executeWithProxyFallback(
                        ...
                        String absolutized = absolutizeManifest(rawManifest, m3u8Url);
        ...
        for (String line : manifest.split("\n")) {
            ...
            if (trimmed.startsWith("http://") || trimmed.startsWith("https://") || trimmed.startsWith("//")) {
                ...
                try {
                    ...
                    result.append(uri.getScheme()).append("://").append(uri.getHost()).append(trimmed).append("\n");
                } catch (Exception e) {
