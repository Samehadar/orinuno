...
[downstream discovery](#downstream consumer-discovery-flow) — can submit work,
...
sequenceDiagram
    autonumber
    participant Client as Client (downstream discovery)
...
downstream consumer discovery **must not** poll
...
[downstream integration → no-polling rule](/orinuno/operations/downstream-integration/#3-no-polling-rule).
