...
description: End-to-end runbook for downstream consumers (downstream consumer first) — pre-flight checks, contract, throughput envelope, failure modes, observability.
---

This page is the single entry point for any downstream service that wants
to integrate with orinuno — primarily
[downstream consumer](https://<private downstream repo>/tree/main/downstream consumer),
