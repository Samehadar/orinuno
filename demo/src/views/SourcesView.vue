...
import { downloadEta, downloadProgressPct, fmtBytes, useJutsuVideo } from '../composables/useJutsuVideo'
import type {
  ContentDto,
...
// jut.su decode + playback + download all live in a shared composable so the JutSu page
// (`/jutsu` → Episode meta tab) reuses the exact same flow without duplicating the CDN
// session-binding workaround. See `docs/quirks-and-hacks.md` → "JutSu DLE auth + sticky cookies".
...
function playInPlayer(url: string) {
  playInPlayerRaw(url, 'jutsu-inline-player')
}
